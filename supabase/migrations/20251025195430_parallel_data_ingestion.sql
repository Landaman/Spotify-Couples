...
CREATE OR REPLACE FUNCTION private.get_album_tracks(album_id text, authorization_header http_header)
 RETURNS SETOF jsonb
 LANGUAGE plpgsql
 PARALLEL SAFE
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION private.header_to_text(header http_header)
 RETURNS text
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
