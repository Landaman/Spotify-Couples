...
BEGIN
  SELECT
    status,
    content::jsonb INTO tracks_status,
    tracks_response
  FROM
    extensions.http (('GET', 'https://api.spotify.com/v1/tracks?ids=' || array_to_string((
        SELECT
          array_agg(DISTINCT id)
        FROM unnest(track_ids) id
  WHERE
    NOT EXISTS (
      SELECT
        1
      FROM public.tracks
      WHERE
	tracks.id = id)), ','), ARRAY[authorization_header],
	  '', '')::extensions.http_request);

  IF tracks_status != 200 THEN
    RAISE EXCEPTION 'InvalidTrackResponse'
      USING detail = 'HTTP Response code: ' || tracks_status || ' body: ' || tracks_response;
    END IF;
    -- Double check to make sure we have the right length
    IF jsonb_array_length(tracks_response -> 'tracks') !=
      ARRAY_LENGTH(track_ids, 1) THEN
      RAISE EXCEPTION 'InvalidTrackResponse'
	USING detail = ('Expected % tracks, only got % from Spotify', ARRAY_LENGTH(track_ids, 1),
	  jsonb_array_elements(tracks_response -> 'tracks'));
      END IF;
      -- Get the artist IDs from each album. We will do tracks below
      artist_ids := (
        SELECT
          array_agg(artist ->> 'id')
        FROM
          jsonb_array_elements(tracks_response -> 'tracks') AS track,
      ...
      FOREACH loop_album IN ARRAY albums LOOP
	    ...
	    extensions.http (('GET', more_url,
		  ...
		  'id')))
              FROM
                jsonb_array_elements(tracks_response -> 'items') AS track);
	    ...
	    IF jsonb_array_length(artists_response -> 'artists') !=
	      ARRAY_LENGTH(artist_ids_chunk, 1) THEN
              RAISE EXCEPTION 'InvalidArtistsResponse'
		USING detail = ('Expected % artists, only got % from Spotify', ARRAY_LENGTH(artist_ids_chunk,
		  1), jsonb_array_length(artists_response -> 'artists'));
              END IF;
